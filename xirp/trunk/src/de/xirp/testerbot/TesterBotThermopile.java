...
public class TesterBotThermopile extends AbstractValueCreator {
	...
	public TesterBotThermopile(long runTime) {
		super(runTime);
		random = new Random( );
		position = new Point(random.nextInt(CELL_COLUMN),
				random.nextInt(CELL_ROW));
		randomTemp = random.nextInt((int) (MAX_CELL_VALUE - HOT_TEMP))
				+ HOT_TEMP;
	}
	 ...
	 */
	@Override
	protected double[][] calculate(@SuppressWarnings("unused")
	long elapsedTime) {
		if (cnt % 10 == 0) {
			randomTemp = random.nextInt((int) (MAX_CELL_VALUE - HOT_TEMP))
					+ HOT_TEMP;
		...
		for (int y = -2; y <= 2; y++) {
			for (int x = -2; x <= 2; x++) {
				if (!(x == 0 && y == 0)) {
					...
					if (tempX >= 0 && tempX < CELL_COLUMN && tempY >= 0
							&& tempY < CELL_ROW) {
						values[tempY][tempX] = randomTemp * factor
								+ (random.nextInt(7) - 3);
					}
				}
			}
