...
public class XirpTextFieldAppender extends AppenderSkeleton {
	...
	private String[] extractRobot(String message) {
		String robot = StringUtils.substringBetween(message,
				RobotLogger.ROBOT_IDENTIFIER,
		...
		String origMessage = StringUtils.substringBefore(message,
				RobotLogger.ROBOT_IDENTIFIER);
		origMessage += StringUtils.substringAfterLast(message,
				RobotLogger.ROBOT_DELIMITER);
	...
	protected void append(final LoggingEvent logEvent) {
		...
		if (mess != null) {
			robotName = mess[0];
			renderedMessage = robotName + ": " + mess[1]; //$NON-NLS-1$
		}
	 ...
	 *            the rendered text
	 */
	private void log(final StyledText textField, final StyleRange range,
