...
/**
 * This dialog is used to open the GUI of a plugin in an own window,
 * or non-plugin panels of the workspace, f.e. the live chart
 * composite.
 * 
...
public final class GenericDialog extends XDialog {
	...
	/**
	 * Constructs a new generic dialog which can display generic
	 * content.
	 * 
	 * @param parent
	 *            The parent shell.
			...
			public void shellClosed(ShellEvent e) {
				...
				if (plugin != null) {
					if (unref) {
						...
						PluginManager.stopPlugin(plugin.getRobotName( ),
	...
	/**
	 * Opens the dialog for the given plugin class name, robot name
	 * and flag indicating if the window should be resizeable.
	 * 
	 * @param className
	 *            The class name.
	 * @param robotName
	 *            The robot name.
	 * @param resizeable
	 *            A flag indicating if the dialog should be
	 *            resizeable.
	 */
	...
	/**
	 * Opens the generic dialog and loads the content for
	 * plugin-origin parts.
	 * 
	 * @param className
	 ...
	 * @param resizeable
	 *            Sets the dialog resizeable <code>true</code>:
	 *            resizeable<br>
	...
	public void open(String className, String key, String robotName,
		...
		if (className != null && robotName != null) {
			try {
				...
				logClass.error(I18n.getString("GenericDialog.log.errorWhileLoadingDialogContent") + Constants.LINE_SEPARATOR); //$NON-NLS-1$
				logClass.error(e.getMessage( ) + Constants.LINE_SEPARATOR);
				logClass.info(I18n.getString("GenericDialog.log.infoMessageConstuctors") + Constants.LINE_SEPARATOR); //$NON-NLS-1$
				dialogShell.close( );
				return;
			}
		}
	 ...
	 * 
	 * @param plugin
	 *            The plugin. may be <code>null</code> if an
	 *            internal panel should be opened.
	 * @param robotName
	 *            the robot name.
	 */
	private void openGenericDialog(final IPlugable plugin,
		...
		if (comp != null) {
			...
			if (plugin != null) {
				dialogShell.setImage(ImageManager.getSystemImage(SystemImage.PLUGIN));
				...
				localeListener = new LocaleChangedListener( ) {
					...
					public void localeChanged(LocaleChangedEvent event) {
						dialogShell.setText(robotName +
								...
								plugin.getHandler( )
										.getString(plugin.getNameKey( )));
					}

				};
			}
			else {
				dialogShell.setImage(ImageManager.getSystemImage(SystemImage.WINDOW));
				dialogShell.setText(robotName + ROBOT_NAME_SEPARATOR +
			...
			logClass.info(I18n.getString("GenericDialog.log.genericDialogCouldNotBeOpened") //$NON-NLS-1$
					+
	...
	/**
	 * Returns the content composite for the given robot name, content
	 * class and parent composite.
	 * 
	 * @param robotName
	 *            The robot name.
	 * @param contentClass
	...
	private Composite getComposite(String robotName,
		...
		try {
			if (robotName == null) {
				Constructor<? extends Composite> constr = contentClass.getConstructor(new Class[] {
	...
	/**
	 * Loads the generic content from the given class name and returns
	 * the content as composite.
	 * 
	 * @param className
	 *            The class to load.
	 * @param parent
	...
	private Composite getComposite(String className, Composite parent)
		...
		Composite comp = null;
		try {
			Class<Composite> clazz = (Class<Composite>) Class.forName(className);
			...
			if (useConstructor == null) {
				throw new InstantiationException("No suitable Constructor found"); //$NON-NLS-1$
			}
			else {
	...
	public void open(String robotName, boolean resizeable) {
		...
		try {
			comp = getComposite(robotName,
					origin.getContentClass( ),
					dialogShell);
		}
