...
public class XCombo extends Combo {
	...
	private void init() {
		...
		localeListener = new LocaleChangedListener( ) {
			...
			LocaleChangedEvent event) {
				// otherwise we will get a concurrent modification
				// exception (when iterating over the maps itself)
				// because items are (pseudo) added on language change
	...
	public void addForLocaleKey(String key, int index, Object... objects) {
		// adding to the end don't need to move items
