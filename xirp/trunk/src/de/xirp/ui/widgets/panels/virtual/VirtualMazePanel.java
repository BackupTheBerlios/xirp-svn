...
public class VirtualMazePanel extends XComposite {

	/**
	 * 
	 */
	private static final String VIRTUAL = "_virtualmaze"; //$NON-NLS-1$

	/**
	 * @param parent
	 * @param style
	 */
	public VirtualMazePanel(Composite parent, int style) {
		super(parent, style);
		init( );
	}
	...
	private void init() {
		...
		DockingComposite docking = new DockingComposite(this,
				"workspacelayout_virtualmaze"); //$NON-NLS-1$
		...
		AbstractLayoutPart part = new LogLayoutPart(LogLayoutPart.class.getCanonicalName( )
				+ VIRTUAL, docking.getDockingManager( ), "virtbotmaze"); //$NON-NLS-1$
		...
		part = new MazeCodeLayoutPart(MazeCodeLayoutPart.class.getCanonicalName( )
				+ VIRTUAL, docking.getDockingManager( ));
		docking.addPart(part);
	}
}
