...
public final class RobotToolbar {
	...
	public RobotToolbar(Composite parent, Robot robot) {
		...
		for (IPlugable plugin : PluginManager.getPluginsForType(robot.getName( ),
				PluginType.COMMUNICATION)) {
			interfaceTriples.add(new KeyClassHandlerTriple(plugin.getNameKey( ),
		...
		for (IPlugable plugin : PluginManager.getPluginsForType(robot.getName( ),
				PluginType.PROTOCOL)) {
			protocolTriples.add(new KeyClassHandlerTriple(plugin.getNameKey( ),
					plugin.getInfo( ).getMainClass( ),
					plugin.getHandler( )));
		}
	...
	private void init() {
		...
		toolItemConnect.addSelectionListener(new SelectionAdapter( ) {
			...
			SelectionEvent e) {
				if (!CommunicationManager.isConnected(robot.getName( ))) {
					try { 
						@SuppressWarnings("unused")
						String interfaceName = comboInterface.getText( );
						@SuppressWarnings("unused")
						String protocolName = comboProtocol.getText( );
						...
						ConnectionObject obj = new ConnectionObject(robot.getName( ),
								interfaceClass,
								protocolClass);
						...
						XMessageBox box = new XMessageBox(parent.getShell( ),
								HMessageBoxType.INFO,
								XButtonType.CLOSE);
						box.setTextForLocaleKey("RobotToolbar.messagebox.title"); //$NON-NLS-1$
						box.setMessageForLocaleKey("RobotToolbar.messagebox.message"); //$NON-NLS-1$
						box.open( );
					}
				}
				else {
		...
		initCombos( ); //Must be called exactly here!
					...
					public boolean filterPlugin(IPlugable plugin) {
						return PluginType.containsType(plugin,
								...
								VisualizationType.containsType(plugin,
										VisualizationType.ROBOT_TOOLBAR);
					}
				});
		...
		// Don't wonder about this calculations, they have to be here
		// otherwise the layout doesn't work (chevrons are shown, even
		// if they are
		// not needed)
	...
	private void setConnected(final boolean connected) {
		SWTUtil.asyncExec(new Runnable( ) {
			...
			public void run() {
				...
				if (connected) {
					toolItemConnect.setToolTipTextForLocaleKey("MainRobotPanelToolbar.gui.disconnectFrom", robot.getName( )); //$NON-NLS-1$
					toolItemConnect.setImage(close);
				}
				else {
					toolItemConnect.setToolTipTextForLocaleKey("MainRobotPanelToolbar.gui.connectTo", robot.getName( )); //$NON-NLS-1$
	...
	class KeyClassHandlerTriple {
		...
		public KeyClassHandlerTriple(String key, String clazz,
				PluginI18NHandler handler) {
			this.key = key;
			this.clazz = clazz;
