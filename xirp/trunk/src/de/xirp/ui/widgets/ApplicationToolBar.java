 ...
 * Changes
 * -------
 * 05.03.2006:		Created by Matthias Gernand.
 */
...
public final class ApplicationToolBar {
	...
	// Logger.getLogger(ApplicationToolBar.class);
	/**
	 * The parent shell.
	 */
	...
	private void createToolBar() {
		...
		GridData gd = SWTUtil.setGridData(coolBar,
				true,
				false,
				SWT.FILL,
				SWT.FILL,
	...
	private void createItems() {
		...
		toolItemQuit.setToolTipTextForLocaleKey("Application.gui.tooltip.quit"); //$NON-NLS-1$
		...
		toolItemSettings.setToolTipTextForLocaleKey("Application.gui.tooltip.settings"); //$NON-NLS-1$
		...
		toolItemStartTimer.setToolTipTextForLocaleKey("ApplicationToolBar.gui.toolTip.startTimer"); //$NON-NLS-1$
		...
		toolItemPauseTimer.setToolTipTextForLocaleKey("ApplicationToolBar.gui.toolTip.pauseTimer"); //$NON-NLS-1$
		...
		toolItemStopTimer.setToolTipTextForLocaleKey("ApplicationToolBar.gui.toolTip.stopResetTimer"); //$NON-NLS-1$
		...
		toolItemMail.setToolTipTextForLocaleKey("ApplicationToolBar.tool.item.sendMail"); //$NON-NLS-1$
		...
		toolItemContacts.setToolTipTextForLocaleKey("ApplicationToolBar.tool.item.contacts"); //$NON-NLS-1$
		...
		toolItemCharts.setToolTipTextForLocaleKey("ApplicationToolBar.tool.item.charts"); //$NON-NLS-1$
	...
	private void createContextMenu() {
		...
		toolBarShow.setTextForLocaleKey("Application.gui.toolbar.toolsVisible"); //$NON-NLS-1$
		...
		toolBarLock.setTextForLocaleKey("ApplicationToolBar.gui.contectmenu.lockTools"); //$NON-NLS-1$
			...
			public void widgetSelected(SelectionEvent e) {
				...
				Runnable runnable = new Runnable( ) {
					...
					public void run() {
						...
						application.getAppMenu( )
								.getSubMenuViewItemLocked( )
								.setSelection(i.getSelection( ));
						parent.layout( );
					}
				};
				SWTUtil.showBusyWhile(parent, runnable);
	...
	private void rearrangeToolBar(Robot robot) {
		...
		if (robot != null) {
			// for (IPlugable plugin :
			// PluginManager.getPluginsForType(robot.getName( ),
			// PluginType.TOOLBAR)) {
			// if (VisualizationType.containsType(plugin,
			// VisualizationType.APP_TOOLBAR)) {
			// XCoolItem parentItem = new XCoolItem(coolBar,
			// SWT.DROP_DOWN);
			// XToolBar bar = plugin.getToolBar(parentItem);
			// bar.setMenu(toolBarPopUp);
			//
			// bar.removeAppearanceListener( );
			// bar.setBackground(coolBar.getBackground( ));
			//
			...
			/*
			 * Don't wonder about this calculations, they have to be
			 * here otherwise the layout doesn't work (chevrons are
			 * shown, even if they are not needed)
			 */
			Point size = toolBar.computeSize(SWT.DEFAULT, SWT.DEFAULT);
			Point coolSize = coolItem.computeSize(size.x, size.y);
			coolItem.setSize(coolSize);
			...
			for (XCoolItem itm : coolItems) {
				Point locSize = itm.getControl( ).computeSize(SWT.DEFAULT,
						SWT.DEFAULT);
