 ...
 * 02-Apr-2007 : Added secure dispose method and assert methods for 
 * 				 swt widgets, resources and devices;           
 */
...
/**
 * Utility class gathering some useful and general methods. Mainly
 * convert forth and back graphical stuff between awt and swt.
 */
public class SWTUtils {
	...
	private static IRessourceProvider provider = new DefaultRessourceProvider( );

	/**
	 * @param provider
	 *            the provider to set
	 */
	public static void setProvider(IRessourceProvider provider) {
		SWTUtils.provider = provider;
	}

	/**
	 * Create a <code>FontData</code> object which encapsulate the
	 * essential data to create a swt font. The data is taken from the
	 * provided awt Font.
	 * <p>
	 * Generally speaking, given a font size, the returned swt font
	 * will display differently on the screen than the awt one.
	 * Because the SWT toolkit use native graphical ressources
	 * whenever it is possible, this fact is plateform dependent. To
	 * address this issue, it is possible to enforce the method to
	 * return a font with the same size (or at least as close as
	 * possible) as the awt one.
	 * <p>
	 * When the object is no more used, the user must explicitely call
	 * the dispose method on the returned font to free the operating
	 * system resources (the garbage collector won't do it).
	 * 
	 * @param device
	...
	public static FontData toSwtFontData(Device device, java.awt.Font font,
		...
		// convert the font size (in pt for awt) to height in pixels
		// for swt
		int height = (int) Math.round(font.getSize( ) * 72.0
				/ device.getDPI( ).y);
		fontData.setHeight(height);
		// hack to ensure the newly created swt fonts will be rendered
		// with the
		// same height as the awt one
		if (ensureSameSize) {
			GC tmpGC = new GC(device);
			Font tmpFont = provider.getFont(fontData);
			// Font tmpFont = new Font(device, fontData);
			tmpGC.setFont(tmpFont);
			if (tmpGC.textExtent(Az).x > DUMMY_PANEL.getFontMetrics(font)
					.stringWidth(Az)) {
				while (tmpGC.textExtent(Az).x > DUMMY_PANEL.getFontMetrics(font)
						.stringWidth(Az)) {
					tmpFont.dispose( );
					height--;
					fontData.setHeight(height);
					tmpFont = provider.getFont(fontData);
					tmpGC.setFont(tmpFont);
				}
			}
			else if (tmpGC.textExtent(Az).x < DUMMY_PANEL.getFontMetrics(font)
					.stringWidth(Az)) {
				while (tmpGC.textExtent(Az).x < DUMMY_PANEL.getFontMetrics(font)
						.stringWidth(Az)) {
					tmpFont.dispose( );
	...
	/**
	 * Create an awt font by converting as much information as
	 * possible from the provided swt <code>FontData</code>.
	 * <p>
	 * Generally speaking, given a font size, an swt font will display
	 * differently on the screen than the corresponding awt one.
	 * Because the SWT toolkit use native graphical ressources
	 * whenever it is possible, this fact is plateform dependent. To
	 * address this issue, it is possible to enforce the method to
	 ...
	 * 
	 * @param device
	 *            The swt device being drawn on (display or gc
	 *            device).
	 * @param fontData
	...
	public static java.awt.Font toAwtFont(Device device, FontData fontData,
		...
		int height = (int) Math.round(fontData.getHeight( )
				* device.getDPI( ).y / 72.0);
		// hack to ensure the newly created awt fonts will be rendered
		// with the
	...
	/**
	 * Create an awt font by converting as much information as
	 * possible from the provided swt <code>Font</code>.
	 * 
	 * @param device
	 *            The swt device to draw on (display or gc device).
	 * @param font
	...
	/**
	 * Creates an awt color instance to match the rgb values of the
	 * specified swt color.
	 * 
	 * @param color
	...
	/**
	 * Creates a swt color instance to match the rgb values of the
	 * specified awt paint. For now, this method test if the paint is
	 * a color and then return the adequate swt color. Otherwise plain
	 * black is assumed.
	 * 
	 * @param device
	 *            The swt device to draw on (display or gc device).
	...
	public static Color toSwtColor(Device device, java.awt.Paint paint) {
		...
		return provider.getColor(color.getRed( ),
				color.getGreen( ),
				color.getBlue( ));
	}
	...
	/**
	 * Creates a swt color instance to match the rgb values of the
	 * specified awt color. alpha channel is not supported. Note that
	 * the dispose method will need to be called on the returned
	 * object.
	 * 
	 * @param device
	 *            The swt device to draw on (display or gc device).
	...
	/**
	 * Transform an awt Rectangle2d instance into a swt one. The
	 * coordinates are rounded to integer for the swt object.
	 * 
	 * @param rect2d
	 ...
	 * @return a Rectangle2D.Double instance with the eappropriate
	 *         location and size.
	 */
	 ...
	 * @return An AWT point with the same coordinates as
	 *         <code>p</code>.
	 ...
	 * @return An SWT point with the same coordinates as
	 *         <code>p</code>.
	...
	/**
	 * Returns an SWT point with the same coordinates as the specified
	 * AWT point (rounded to integer values).
	 * 
	 * @param p
	 *            the AWT point (<code>null</code> not permitted).
	 * @return An SWT point with the same coordinates as
	...
	/**
	 * Creates an AWT <code>MouseEvent</code> from a swt event. This
	 * method helps passing SWT mouse event to awt components.
	 * 
	 * @param event
	 *            The swt event.
	...
	/**
	 * Disposes the given widget only if it is not null or not
	 * disposed.
	 * 
	 * @param widget
	 *            The widget to dispose.
	 * @return boolean <code>true</code> if dispose was successfull<br>
	 *         <code>false</coed> if dispose was not successfull<br>
	 */
	...
	/**
	 * Disposes the given device only if it is not null or not
	 * disposed.
	 * 
	 * @param device
	 *            The device to dispose.
	 * @return boolean <code>true</code> if device was successfull<br>
	 *         <code>false</coed> if device was not successfull<br>
	 */
	...
	/**
	 * Disposes the given resource only if it is not null or not
	 * disposed.
	 * 
	 * @param resource
	 *            The widget to dispose.
	...
	/**
	 * Tests if a widget is not null and not disposed.
	 * 
	 * @param widget
	 *            The widget to test.
	 * @return boolean <code>true</code> if widget is not null and
	 *         not disposed.<br>
	 *         <code>false</code> if widget is either null or
	 *         disposed.<br>
	 */
	public static boolean swtAssert(Widget widget) {
		return (widget != null && !widget.isDisposed( ));
	}

	/**
	 * Tests if a device is not null and not disposed.
	 * 
	 * @param device
	 *            The device to test.
	 * @return boolean <code>true</code> if device is not null and
	 *         not disposed.<br>
	 *         <code>false</code> if device is either null or
	 *         disposed.<br>
	 */
	public static boolean swtAssert(Device device) {
		return (device != null && !device.isDisposed( ));
	}

	/**
	 * Tests if a resource is not null and not disposed.
	 * 
	 * @param resource
	 *            The resource to test.
	 * @return boolean <code>true</code> if resource is not null and
	 *         not disposed.<br>
	 *         <code>false</code> if resource is either null or
	 *         disposed.<br>
	 */
	public static boolean swtAssert(Resource resource) {
